...
public class FundController {
    ...
    @GetMapping("/{fundCode}")
    public FundVo getByCode(@PathVariable String fundCode) {
        return fundService.getByCode(fundCode);
    }

    /** POST /api/funds - 펀드 생성 */
    @PostMapping 
    public FundVo create(@RequestBody FundVo vo) { 
        return fundService.save(vo); 
    }

    /** PUT /api/funds/{fundCode} - 펀드 수정 */
    @PutMapping("/{fundCode}") 
    public void update(@PathVariable String fundCode, @RequestBody FundVo vo) {
    ...
    public List<FundPerformanceVo> getPerformance(@PathVariable String fundCode) {
    ...
    public List<FundPerformanceVo> getPerformanceByDateRange(
    ...
    public void savePerformance(@PathVariable String fundCode, @RequestBody FundPerformanceVo performanceVo) {
        performanceVo.setFundCode(fundCode);
        fundService.savePerformance(performanceVo);
    }

    /** GET /api/funds/top/{limit} - 상위 성과 펀드 조회 */
    @GetMapping("/top/{limit}")
    public List<FundVo> getTopPerformingFunds(@PathVariable int limit) {
        return fundService.getTopPerformingFunds(limit);
    }

    /** GET /api/funds/type/{fundType} - 펀드 유형별 조회 */
    @GetMapping("/type/{fundType}")
    public List<FundVo> getFundsByType(@PathVariable String fundType) {
        return fundService.getFundsByType(fundType);
    }

    /** GET /api/funds/risk/{riskLevel} - 위험도별 펀드 조회 */
    @GetMapping("/risk/{riskLevel}")
    public List<FundVo> getFundsByRiskLevel(@PathVariable String riskLevel) {
    ...
    public FundAnalysisResultVo analyzeFund(@PathVariable String fundCode, @RequestParam(defaultValue = "1Y") String period) {
